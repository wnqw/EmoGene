...
class MEADSeqDataset(Dataset):
    def __init__(self, prefix='train'):
        ...
        self.emo_map = {'ang':'angry', 'dis':'disgust', 'con':'contempt', 'fea':'fear', 'hap':'happy', 'sad':'sad', 'sur':'surprise', 'neu':'neutral'}
    ...
    def __getitem__(self, idx):
        ...
        # Load lm3d
        t_lm, dim_lm, _ = raw_item['idexp_lm3d'].shape # [T, 68, 3]
        item['idexp_lm3d'] = torch.from_numpy(raw_item['idexp_lm3d']).reshape(t_lm, -1).float() # [T, 204]
        # selected_data = raw_item['idexp_lm3d'][:, :64, 0]
        # item['idexp_lm3d'] = torch.from_numpy(selected_data).float() # [T, 64]
        ...
        # load emotion labels
        text_emotion = [self.emo_map[raw_item['emotion']]]
        # print(text_emotion)
        int_emo_encoded = self.label_encoder.transform(text_emotion)
        item['emotion'] = torch.from_numpy(int_emo_encoded) # int tensor
        
        # item = self.memory_cache[idx]
        item['ref_mean_lm3d'] = item['idexp_lm3d'].mean(dim=0).reshape([204,])        
        # item['ref_mean_lm3d'] = item['idexp_lm3d'].mean(dim=0).reshape([64,])
        return item
    ...
    def collater(self, samples):
        ...
        emotion = torch.stack([s['emotion'] for s in samples], dim=0) # [b, h=204*5]
    ...
    def get_dataloader(self):
        ...
        loader = DataLoader(self, pin_memory=False,collate_fn=self.collater, batch_sampler=batches_idx, num_workers=4) # num_workers=4
        return loader
